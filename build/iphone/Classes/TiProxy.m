...
//Rare exceptions to indicate a bug in the _nexus code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_nexus = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_nexus]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.nexus.module"
